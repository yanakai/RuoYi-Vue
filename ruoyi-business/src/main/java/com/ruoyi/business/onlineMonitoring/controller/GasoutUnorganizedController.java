...
@RestController
@RequestMapping("/business/onlinemonitoring/gasoutPutUnorganized")
public class GasoutUnorganizedController extends BaseController {
    @Autowired
    ...
    @GetMapping("/yc/list")
    public TableDataInfo ycList(GasoutDTO gasoutDTO) {
         try {
    ...
    @ApiOperation("废气排口在线监测")
    @GetMapping("/voc/list")
    public TableDataInfo vocList(GasoutDTO gasoutDTO) {
        try {
            return gasoutService.selectDataList(gasoutDTO);
        }catch (Exception e){
            log.error("排口查询错误",e);
